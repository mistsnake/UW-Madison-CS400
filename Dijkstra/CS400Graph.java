...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
        	...
        	this.dataSequence = new LinkedList<>();
        	for(T currElem : copyPath.dataSequence) {
        		this.dataSequence.add(currElem);
        	}
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
    	//Check provided input for validity; can't do anything with any null values
    	//or if the vertex doesn't exist (when no vertex containing the start or end can be found)
    	if(start == null || end == null)
    		throw new NoSuchElementException();
    	if (!containsVertex(start) || !containsVertex(end))
    		throw new NoSuchElementException();
    	
    	//create priority queue and an arraylist to keep track of found paths and vertices respectively
    	PriorityQueue<Path> paths = new PriorityQueue<Path>();
    	ArrayList<Vertex> visitedVertices = new ArrayList<Vertex>();
    	
    	//initialize temp with the start vertex and add to the paths for beginning of while loop 
    	//this will later become used as a temporary value in loop
    	...
    	//go through priority queue and create new Paths from the unvisited vertices of the
    	//edges of each priority queue Path and add them to the priority queue until
    	//all vertices have been visited or until a Path between the start and end 
    	//values are found
    	
    	while(paths.size() != 0) {
            	
    		//obtain the current Path to check from the priority queue
    		temp = paths.poll();
    		
    		//once a path has been found between the indicated vertices, stop while loop
    		//and set the return value
        	if(temp.end.data.equals(end) && temp.start.data.equals(start)) {
        		retPath = temp;
        		break;
        	}
        	
        	//create new path for use inside for loop, keeps the new paths created to be inserted into
        	//PriorityQueue
        	Path newPath = null;  
        	
        	//check the current Path's edges
    		for(Edge currEdge : temp.end.edgesLeaving) {
    			//For each unvisited vertex, create a new Path that ranges from the first
    			//start vertex given to the current edge's target
    			if(!visitedVertices.contains(currEdge.target)) {
    				newPath = new Path(temp, currEdge);
    				visitedVertices.add(temp.end);	
    				paths.add(newPath);
    			}
    		}	
    	}
    	
    	//if there is no path to be found, throw exception
    	if(retPath == null)
    		throw new NoSuchElementException();
